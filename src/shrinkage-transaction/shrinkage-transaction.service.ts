...
export class ShrinkageTransactionService extends BaseService<ShrinkageTransactionDoc> {
  constructor(
    @InjectModel(ShrinkageTransaction.name)
    readonly m: Model<ShrinkageTransactionDoc>,
  ...
  async createShrinkageTransactions(
    ...
    await session.withTransaction(async (): Promise<any> => {
      for (const item of dto.items) {
        ...
        if (item.batches) {
          ...
          for (const batch of item.batches) {
            const selectedBatch =
              await this.stockItemService.stockItemTransaction.findOne(
                {
                ...
                {
                  qtyRemaining: 1,
                  batches: { $elemMatch: { _id: this.toObjectId(batch._id) } },
                },
              );
              ...
              },
              {
                ...
                $set: {
                  'batches.$.qtyRemaining':
                    selectedBatch.batches[0].qtyRemaining - item.shrinkQty,
                },
              },
          ...
          const sortedDateBatches =
            await this.stockItemService.stockItemTransaction.aggregate([
              {
                $match: {
                  stockItemId: item.stockItemId,
                  qtyRemaining: { $ne: 0 },
      ...
      stockitemShrinked = await this.m.create(
        [
          {
