...
export class SuppliersService extends BaseService<SupplierDoc> {
  ...
  async createSupplier(req: RequestWithUser, dto: CreateSupplierDto) {
    return await this.create({ ...dto, entityId: req.user.entityId });
  }
  ...
  async downloadAll(res: Response, options: DownloadSupplierSearchOptions) {
    const aggregation = [];

    const { dir, searchTerm, filterBy, attributesToRetrieve, filterByDateFrom, filterByDateTo } =
      options;
