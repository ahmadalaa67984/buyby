...
import { EmailTakenException, RecordNotFoundException } from 'src/core/exceptions';
...
import { generateCode, generateUniqueNumericCode, isStillValid, toObjectId } from 'src/core/utils';
...
export class UsersService extends BaseService<UserDoc> {
  ...
  async me(req: RequestWithUser): Promise<User> {
    const user = await (
      await this.findOneById(req.user._id, { password: 0, pin: 0 })
    ).populate('entityId');
  ...
  async updateMe(req: RequestWithUser, dto: UpdateUserDto): Promise<User> {
    ...
    if (dto.phoneNumber !== user.phoneNumber) {
      checkPhone = await this.findOne(
        { phoneNumber: dto.phoneNumber, _id: { $ne: req.user._id } },
        { password: 0, pin: 0 },
      );
      // console.log('check', checkPhone);
    }
    if (!checkPhone) {
      const updatedData = await this.update(req.user._id, dto, {
        password: 0,
        pin: 0,
      });
  ...
  async updateCustomerPromotional(
    req: RequestWithUser,
    id: string,
    dto: UpdateCustomerPromotionalDto,
  ) {
    const user = await this.findOne(
  ...
  async createWithPassword(signUpDto: CreateUserDto) {
    ...
    const user = await this.create({ type: type, ...signUpDto, sku });
    ...
    const tokenData = await this.emailToken.create({
      userId: user._id,
  ...
  async sendReverifyEmail(emailDto: EmailDto) {
    ...
    const tokenData = await this.emailToken.create({
      userId: user._id,
      type: TokenType.EMAIL_REVERIFICATION_TOKEN,
    });
  ...
  async findMyActiveUsers(options: UserSearchOptions, req: RequestWithUser): Promise<Pagination> {
    const aggregation = [];
    ...
    aggregation.push({ $match: { role: { $ne: RolesEnum.CUSTOMER } } });
    ...
    aggregation.push({
      $match: {
        $and: [{ owner: { $eq: req.user._id } }, { active: { $eq: true } }],
      },
    });
  ...
  async findMyDeActiveUsers(options: UserSearchOptions, req: RequestWithUser): Promise<Pagination> {
    const aggregation = [];
    ...
    if (sortValue && dir) {
      this.sort(aggregation, sortValue, dir);
    }
    ...
    aggregation.push({
      $match: {
        $and: [{ owner: { $eq: req.user._id } }, { active: { $eq: false } }],
      },
    });
  ...
  async deactiveUser(req: RequestWithUser, id: string, dto: DeactiveUserDto) {
    ...
    const userIds = [];
    const user = await this.findOneById(toObjectId(id), {
      password: 0,
      pin: 0,
    });
    ...
    if (!user) throw new NotFoundException('user not fount');
      ...
      if (userIds) {
        await this.updateMany({ _id: { $in: userIds } }, dto, {
          password: 0,
          pin: 0,
        });
      }
  ...
  async createProfile(req: RequestWithUser, dto: CreateProfileDto) {
    ...
    if (packageExist.users.length >= packageLimits.limit + req.user.extraLimit)
    ...
    const { sku } = await this.entityService.findOne({ _id: req.user.entityId });
  ...
  async handleCron2() {
    ...
    const user = await this.findOne({
      expireOn: '2022-11-15T09:12:18.666+00:00',
    });
  ...
  async getBusinessCustomerUser(options: UserSearchOptions) {
    const aggregation = [];
  ...
  async getBusinessUser(options: UserSearchOptions) {
    const aggregation = [];
    ...
    const allBusinessOwners = await this.find({}, { price: 1, _id: 0 });
    let totalPrice = 0;
    allBusinessOwners.map(({ price }) => (totalPrice += price));
    const data = await this.aggregate(aggregation, offset, size);
