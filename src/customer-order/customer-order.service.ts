...
export class CustomerOrderService extends BaseService<OrderDoc> {
  ...
  async createOrder(req: RequestWithUser, dto: CreateCustomerOrderDto) {
    ...
    try {
      ...
      entityExist = await this.entitiesService.findOneAndErr({
        _id: this.toObjectId(dto.entityId),
      });
      warehouseExist = await this.warehouseService.findOneAndErr({
        owner: this.toObjectId(entityExist.owner),
      });
      ...
      await session.withTransaction(async (): Promise<any> => {
        ...
        totalPriceAndDiscountId = await this.orderService.calculateTotalPrice(
          req,
          subtotalItems,
          subtotalServices,
          entityExist.vat,
