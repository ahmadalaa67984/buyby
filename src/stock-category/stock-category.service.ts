...
import {
  StockCategory,
  StockCategoryDoc,
} from './entities/stock-category.entity';

@Injectable()
export class StockCategoryService extends BaseService<StockCategoryDoc> {
  constructor(
    @InjectModel(StockCategory.name)
    private readonly m: Model<StockCategoryDoc>,
  ) {
    super(m);
  }
  ...
  private search(aggregation: any, searchTerm: string): void {
    aggregation.push({
      $match: {
        stockategoryName: { $regex: new RegExp(searchTerm), $options: 'i' },
      },
    });
  }
}
