...
export class QuotationService extends BaseService<QuotationDoc> {
  ...
  async createQuotation(req: RequestWithUser, dto: CreateQuotationDto, res: Response) {
    ...
    for (const item in dto.itemsList) {
      const stockItem = await this.stockItemService.findOneById(dto.itemsList[item].stockItemId);
      const entity = await this.entitiesService.findOneById(stockItem.entityId);
      if (stockItem) {
        ...
        itemData = {
          name: name,
          sellingPrice: dto.itemsList[item].sellingPrice, // * send selling price from body
          storeName: entity.name,
          storePhoneNumber: entity.phoneNumber,
    ...
    this.mailService.sendQuotationMail(dto.customerEmail, data);
    res.status(200).json({ data: quotation });
  }

  async deleteQuotation(req: RequestWithUser, id: string) {
    const quotationExist = await this.removeOne({
  ...
  async findQuotation(req: RequestWithUser, id: string) {
    const quotationExist = await this.findOne({
      _id: this.toObjectId(id),
      userId: req.user._id,
    });
    if (!quotationExist) throw new BadRequestException('025,R025');
