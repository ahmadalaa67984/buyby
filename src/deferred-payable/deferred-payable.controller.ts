...
export class DeferredPayableController {
  ...
  async updateDeferredPayable(
    ...
    @Param('id') id: string,
    @Body() dto: UpdateDeferredPayableDto,
  ) {
    return await this.DeferredPayableService.updateDeferredPayable(
      req,
      id,
      dto,
    );
  ...
  async search(
    @Req() req,
    @Body() options: DeferredPayableSearchOptions,
  ): Promise<Pagination> {
    return await this.DeferredPayableService.findAll(
      req.user._id,
      options,
      req,
    );
