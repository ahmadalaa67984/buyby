...
import {
  DeferredPayable,
  DeferredPayableDoc,
} from './entities/deferred-payable.entity';
...
export class DeferredPayableService extends BaseService<DeferredPayableDoc> {
  ...
  async updateDeferredPayable(
    req: RequestWithUser,
    id: string,
    dto: UpdateDeferredPayableDto,
  ) {
    const payableDeferredExist = await this.findOneById(id);
    if (!payableDeferredExist)
    ...
    if (payableDeferredExist.deferredPayableAmount - dto.payableAmount < 0)
      throw new BadRequestException('018,R018');
    const deferredPayable = await this.update(id, {
      deferredPayableAmount:
        payableDeferredExist.deferredPayableAmount - dto.payableAmount,
      notes: dto.notes,
    });

    const payableExist = await this.payableService.findOne({
      transactionID: deferredPayable.transactionID,
    });
    const Payable = await this.payableService.updateOne(
      { transactionID: deferredPayable.transactionID },
      {
