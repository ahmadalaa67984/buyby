...
export class OrderService extends BaseService<OrderDoc> {
  ...
  async retrieveQtyOfOnHoldItems(warehouseId, items) {
    ...
    try {
      for (const item in items) {
        const stockItem = await this.stockItemService.findOneById(items[item].stockItemId);
        ...
        updatedWarehouseStockItems = await this.stockItemService.warehouseStockItems.updateOne(
          { _id: this.toObjectId(warehouseItem._id) },
          { qtyOnHold: newQtyOnHold },
        );
      }
    ...
    try {
      for (const returnedItem in dto.returnedItems) {
        ...
        if (stockItem) {
          ...
          let totalReturnedPrice =
            dto.returnedItems[returnedItem].qty * dto.returnedItems[returnedItem].unitPrice;
  ...
  async returnedServicesOrder(req: RequestWithUser, dto) {
    ...
    try {
      for (const returnedService in dto.returnedServices) {
        ...
        if (servicesExist) {
          let totalReturnedPrice =
            dto.returnedServices[returnedService].qty *
            dto.returnedServices[returnedService].unitPrice;
          ...
          notes = dto.returnedServices[returnedService].notes
            ? dto.returnedServices[returnedService].notes
            : '';
  ...
  async returnedTransactionForReturnedOrder(req: RequestWithUser, items, customerId) {
    ...
    try {
      for (const item in items) {
        ...
        refund = await this.refundTransactionsService.createRefundTransactions(req, {
          ...returnedData,
        });
  ...
  async releaseActiveItemsForReturnedOrder(req: RequestWithUser, orderExistId, items, customerId) {
    ...
    try {
      for (const item in items) {
        const target = await this.findOne(
          ...
          },
          {
            items: {
              $elemMatch: {
                stockItemId: this.toObjectId(items[item].stockItemId),
              },
            },
          },
        );
      ...
      release = await this.releaseTransactionsService.createReleaseTransactions(req, { ...data });
      return release;
    } catch (error) {
      throw error;
    }
    ...
    try {
      for (const refundedItem in dto.refundedItems) {
        ...
        if (stockItem) {
          ...
          let totalReturnedPrice =
            dto.refundedItems[refundedItem].qty * dto.refundedItems[refundedItem].unitPrice;
  ...
  async refundedServicesOrder(req: RequestWithUser, dto) {
    ...
    try {
      for (const refundedService in dto.refundedServices) {
        ...
        if (servicesExist) {
          let totalReturnedPrice =
            dto.refundedServices[refundedService].qty *
            dto.refundedServices[refundedService].unitPrice;
          qty = dto.refundedServices[refundedService].qty;
          unitPrice = dto.refundedServices[refundedService].unitPrice;
          totalUnitPrice = totalReturnedPrice;
          name = servicesExist.name;
          notes = dto.refundedServices[refundedService].notes
            ? dto.refundedServices[refundedService].notes
            : '';
          serviceId = this.toObjectId(servicesExist._id);
  ...
  async calculateTotalPriceForReturnedItems(
    req: RequestWithUser,
    subtotal,
    entityVat,
    entityService,
    ...
    if (discountValue > 0) discountExist = await this.couponService.findOneById(discountId);
  ...
  async calculateQtyOfOnHoldItems(warehouseId, items) {
    ...
    try {
      for (const item in items) {
        const stockItem = await this.stockItemService.findOneById(items[item].stockItemId);
        // console.log('item', stockItem, warehouseId);
        const warehouseItem = await this.warehouseStockItems.findOne({
          _id: stockItem.warehouseStockItemsData,
          warehouseId: warehouseId,
        });
  ...
  async calculateTotalPrice(
    ...
    let vat = this.calculatePriceAfterVat(subTotalItems, entityVat);
    let service = this.calculatePriceAfterService(subTotalItems, entityService);
    let totalPriceWithService = subTotalItems + subTotalServices + vat + service;
    ...
    if (discountExist) {
      ...
      const discountLimit = await this.couponService.update(discountExist._id, {
        $push: { userIds: customerId },
      });
  ...
  async createOrder(req: RequestWithUser, dto: CreateOrderDto) {
    ...
    if (cart) await this.cartService.m.updateOne({ _id: cart._id }, { cartItems: [], subTotal: 0 });

    let releasedData, onHoldData, status, discountId, tableId, tableExist;
    if (req.user.active !== true) throw new BadRequestException('012,R012');

    //  If user is not owner should search it's entity
    ...
    const entityExist = await this.entitiesService.findOneById(entityId);
    //items

    const items = await this.orderItems(entityExist._id, dto);
    // console.log('m', items);
    ...
    //services
    const services = await this.servicesOrder(entityExist._id, dto);
    ...
    const totalPriceAndDiscountId = await this.calculateTotalPrice(
      req,
      subtotalItems,
      subtotalServices,
      entityExist.vat,
    ...
    const order = await this.create({
      ...
      items: items,
      services: services,
      totalOrder: parseFloat(subtotalItems.toFixed(2)),
      vat: vat,
      service: service,
      totalPriceWithVatAndService: totalPriceBeforeDiscount.toFixed(2),
      discountValue: discountValue ? discountValue.toFixed(2) : 0.0,
  ...
  async findOrder(req: RequestWithUser, id: string) {
    ...
    const result = await this.aggregateOne([
      ...
      // todo: add customerData from customer
      {
        $lookup: {
          from: 'users',
          localField: 'customerId',
  ...
  async voidedOrder(req: RequestWithUser, id: string, orderStatus: string) {
    ...
    for (const refundedItem in OrderExist.items) {
      ...
      if (stockItem) {
        ...
        throw new BadRequestException('016,R016');
      }
    }
    const returnedData = {
      warehouseId: this.toObjectId(req.user.warehouseId),
  ...
  async refundOrder(req: RequestWithUser, id: string, dto: RefundedOrderDto) {
    // cancelled part or full order and take money
    if (req.user.status !== StatusEnum.COMPLETED) throw new BadRequestException('140,R140');
    const OrderExist = await this.findOneAndErr({
      // userId: this.toObjectId(req.user._id),
      _id: this.toObjectId(id),
    ...
    let result,
      ...
      releasedData,
      discountId,
      totalPriceAndDiscountId;
    let subtotalItems = 0,
      subtotalServices = 0,
      subtotalRefundedItems = 0,
      subtotalRefundedServices = 0,
      totalPriceBeforeDiscount = 0,
      discountValue = 0,
      vat = 0,
      service = 0,
    ...
    if (dto.refundMethod == ReturnMethodEnum.PARTIALLY) {
      ...
      refundedTransaction = await this.refundedTransaction(
        req,
        refundedItems,
        OrderExist.customerId,
      );

      subtotal =
        OrderExist.totalOrder +
        subtotalItems +
        subtotalServices -
        subtotalRefundedItems -
        subtotalRefundedServices;
      ...
      totalPriceAndDiscountId = await this.calculateTotalPriceForReturnedItems(
        req,
        subtotal,
        entityExist.vat,
        entityExist.service,
        OrderExist.discountValue,
        OrderExist.discountId,
      );
      totalPrice = totalPriceAndDiscountId.totalPrice;
      totalPriceBeforeDiscount = totalPriceAndDiscountId.totalPriceWithService;
      discountValue = totalPriceAndDiscountId.discountValue;
  ...
  async returnOrder(req: RequestWithUser, id: string, dto: ReturnedOrderDto) {
    // cancelled part or full order and take anothers
    if (req.user.status !== StatusEnum.COMPLETED) throw new BadRequestException('140,R140');
    const OrderExist = await this.findOneAndErr({
      // userId: this.toObjectId(req.user._id),
      _id: this.toObjectId(id),
    ...
    let result,
      ...
      releasedItems,
      releasedData,
      releasedPartiallyData,
      discountId,
      totalPriceAndDiscountId;
    let subtotalItems = 0,
      subtotalServices = 0,
      subtotalReturnedItems = 0,
      subtotalReturnedServices = 0,
      totalPriceBeforeDiscount = 0,
      discountValue = 0,
      vat = 0,
      service = 0,
    ...
    returnedTransaction = await this.returnedTransactionForReturnedOrder(
      req,
      returnedItems,
      OrderExist.customerId,
    );
    ...
    if (dto.returnMethod == ReturnMethodEnum.PARTIALLY) {
      ...
      releasedPartiallyData = await this.releaseActiveItemsForReturnedOrder(
        req,
        OrderExist._id,
        items,
        OrderExist.customerId,
  ...
  async cancelOnHoldOrder(req: RequestWithUser, id: string) {
    ...
    for (const item in OrderExist.items) {
      const stockItem = await this.stockItemService.findOneById(OrderExist.items[item].stockItemId);
      // console.log('stockItem', stockItem);
  ...
  async activeOnholdOrder(req: RequestWithUser, id: string, dto: ActiveOnHoldOrderDto) {
    ...
    //calculate price
    // subtotal = subtotalItems + subtotalServices;
    totalPriceAndDiscountId = await this.calculateTotalPrice(
      req,
      subtotalItems,
      subtotalServices,
      entityExist.vat,
    ...
    const finalResult = await this.aggregateOne([
      ...
      { $addFields: { customerData: { $concatArrays: ['$cusData', '$userData'] } } },
      {
        $lookup: {
          from: 'entities',
          localField: 'entityId',
   ...
   * Search Orders collection.  //done
   */
  async findAll(entityId: Types.ObjectId, options: OrderSearchOptions): Promise<Pagination> {
    const aggregation = [];
  ...
  async checkPin(req: RequestWithUser, dto: CheckPinDto) {
    const businessOwner = await this.usersService.findOneAndErr({
      _id: this.toObjectId(req.user.owner),
    });
  ...
  async orderCompleted(req: RequestWithUser, id: string) {
    const order = await this.findOneAndErr({
      _id: this.toObjectId(id),
      entityId: this.toObjectId(req.user.entityId),
      status: StatusEnum.PENDING,
    });
    ...
    if (order.status === StatusEnum.COMPLETED) {
      // what is the Error code should be? ..
      throw new BadRequestException('140,R140');
    }

    const result = await this.update(id, { status: StatusEnum.COMPLETED });
  ...
  async sendOrderByEmail(dto: EmailingOrderDto) {
    try {
      const { customerEmail, orderLink, storeName } = dto;
      return this.mailService.sendOrder(customerEmail, orderLink, storeName);
    } catch (err: unknown) {
      if (err instanceof Error) throw new BadGatewayException(err.message);
    }
  }

  async topItemsSold(req: RequestWithUser, dto: TopItems, entityId?: Types.ObjectId) {
    let aggregation = [];
    ...
    let topItems = await this.m.aggregate(aggregation);
