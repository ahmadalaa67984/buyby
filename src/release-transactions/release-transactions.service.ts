...
export class ReleaseTransactionsService extends BaseService<ReleaseTransactionsDoc> {
  constructor(
    @InjectModel(ReleaseTransactions.name)
    readonly m: Model<ReleaseTransactionsDoc>,
  ...
  async calculatePrice(releaseQty: number, stockItem, trackBatch: boolean, session: ClientSession) {
    ...
    for (const stockItemDoc of stockItem) {
      // console.log('stock item', stockItemDoc);
      ...
      if (releaseQty <= qtyRemaining && releaseQty != 0) {
        ...
        if (trackBatch) {
          // console.log('inside track batices', stockItemDoc);
          if (stockItemDoc?.batches)
  ...
  async createReleaseTransactions(
    ...
    await session.withTransaction(async (): Promise<any> => {
      ...
      // console.log('stockitems', dto);

      for (const item of stockitems) {
        // console.log('from stokitems in create Relase transaction -><', item);
        const stockItem = await this.stockItemService.findOneAndErr({
      ...
      stockItemsReleased = await this.m.create(
        {
