...
export class BaseService<T> {
  ...
  public filterByPeriod(aggregation: any, period: Period) {
    if (period.period == PeriodEnum.YESTERDAY) {
      const nowDate = moment(period.date).subtract(1, 'day').format('YYYY-MM-DD');
      aggregation.push(
        {
          $addFields: {
            createdAtToString: {
   ...
   */
  public filter(aggregation: Record<string, any>[], filterBy: Record<string, any>[]): void {
    const matchQry = [];
   ...
   */
  public project(aggregation: Record<string, any>[], attributes: string[]): void {
   ...
   */
  public paginate(aggregation: Record<string, any>[], offset: number, size: number): void {
   ...
   */
  async findOneById(id: string | Types.ObjectId, projection: any = {}): Promise<T> {
    const doc = await this.model.findById(this.toObjectId(id), projection).exec();
    if (!doc) {
      throw new NotFoundException('002,R002');
    }
  ...
  async update(
    ...
    const doc = await this.model.findByIdAndUpdate(this.toObjectId(id), updates, {
      new: true,
      projection,
    });
    if (!doc) {
   ...
   */
  async aggregate(aggregation: any[], offset: number, size: number): Promise<Pagination> {
    aggregation.push(
      {
        $group: {
          _id: null,
