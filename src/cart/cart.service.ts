...
export class CartService extends BaseService<CartDoc> {
  ...
  async createCart(req: RequestWithUser, dto: CreateCartDto) {
    ...
    for (const item in dto.cartItems) {
      const stockItem = await this.stockItemService.findOneById(dto.cartItems[item].stockItemId);
  ...
  async addItemsToCart(req: RequestWithUser, dto: AddItemDto) {
    ...
    const Cart = await this.findOne({
      customerId: this.toObjectId(req.user._id),
    });
    ...
    if (Cart.entityId.equals(this.toObjectId(stockItem.entityId))) {
      ...
      if (Cart.cartItems.length > 0) {
        ...
        if (target) {
          ...
          // console.log('1');
          result = await this.update(Cart._id, {
            subTotal: subTotal,
            entityId: this.toObjectId(stockItem.entityId),
  ...
  // * set stock item
  async setItemsToCart(req: RequestWithUser, dto: AddItemDto) {
    const Cart = await this.findOne({
      customerId: this.toObjectId(req.user._id),
    });
    ...
    if (!stockItem) throw new NotFoundException('026,R026');

    const item = Cart.cartItems.filter(
      (item) => item.stockItemId.toString() === dto.stockItemId.toString(),
    );
    const totalPrice = item.length > 0 ? item[0].itemTotal : 0;
    ...
    if (dto.qty == 0) {
      ...
      await this.updateOne(
        { _id: Cart._id },
        {
          $pull: { cartItems: { stockItemId: this.toObjectId(dto.stockItemId) } },
          subTotal: Cart.subTotal - totalPrice,
        },
      );
    } else {
      // console.log('from qty');
      const warehouseItem = await this.warehouseStockItems.findOne({
        _id: stockItem.warehouseStockItemsData,
      });

      // console.log('warehou', warehouseItem);

      const sellingPrice = warehouseItem.sellingPrice;
      const nameLocalized = stockItem.nameLocalized;
      const stockItemId = stockItem._id;
      const img = stockItem.picture;
      const qty = dto.qty;
      ...
      await this.updateOne(
        { _id: Cart._id },
        {
          $pull: { cartItems: { stockItemId: this.toObjectId(dto.stockItemId) } },
          // $push: { cartItems: CartItem },
          // subTotal: Cart.subTotal - totalPrice + itemTotal,
        },
      );
      ...
      await this.updateOne(
        { _id: Cart._id },
        {
          // $pull: { cartItems: { stockItemId: dto.stockItemId } },
          $push: { cartItems: CartItem },
          subTotal: Cart.subTotal - totalPrice + itemTotal,
        },
      );
    ...
    aggregation.push(
      {
        $match: { _id: this.toObjectId(Cart._id) },
      },
      {
        $lookup: {
          from: 'entities',
    ...
    result = await this.updateOne(
      ...
      },
      {
        $set: {
          ...
          'cartItems.$.itemTotal':
            target.cartItems[0].itemTotal + 1 * target.cartItems[0].sellingPrice,
        },
        subTotal: subTotal,
      },
    );
