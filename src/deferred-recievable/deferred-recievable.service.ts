...
import {
  DeferredRecievable,
  DeferredRecievableDoc,
} from './entities/deferred-recievable.entity';

@Injectable()
export class DeferredRecievableService extends BaseService<DeferredRecievableDoc> {
  constructor(
    @InjectModel(DeferredRecievable.name)
    readonly m: Model<DeferredRecievableDoc>,
  ...
  async updateDeferredRecievable(
    req: RequestWithUser,
    id: string,
    dto: UpdateDeferredRecievableDto,
  ) {
    const recievableDeferredExist = await this.findOneById(id);
    if (!recievableDeferredExist)
      throw new BadRequestException('this record is not exist');
    ...
    if (
      recievableDeferredExist.deferredRecievableAmount - dto.recievableAmount <
      0
    )
      throw new BadRequestException('018,R018');

    const deferredRecievable = await this.update(id, {
      deferredRecievableAmount:
        recievableDeferredExist.deferredRecievableAmount - dto.recievableAmount,
      notes: dto.notes,
    });

    const recievableExist = await this.recievableService.findOne({
      transactionID: deferredRecievable.transactionID,
    });

    const Recievable = await this.recievableService.updateOne(
      { transactionID: deferredRecievable.transactionID },
      {
        recievableAmount:
          recievableExist.recievableAmount + dto.recievableAmount,
      },
    );
