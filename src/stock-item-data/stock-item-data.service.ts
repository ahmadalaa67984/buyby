...
export class StockItemDataService extends BaseService<StockItemDoc> {
  ...
  async prepareCSV(stockItems: StockItemDoc[], sheetName: string, res: Response) {
  ...
  async createStockItem(req: RequestWithUser, dto: CreateStockItemDto) {
    ...
    for (const name in dto.nameLocalized) {
      const stockCategory = await this.stockCategoryService.find({
        stockategoryName: {
          $in: [
            dto.nameLocalized.mainLanguage,
            dto.nameLocalized.secondLanguage,
            dto.nameLocalized.thirdLanguage,
          ],
        },
      });
      ...
      for (const stockName in stockCategory) {
        if (
          stockCategory[stockName].stockategoryName == dto.nameLocalized.mainLanguage ||
          dto.nameLocalized.secondLanguage ||
          dto.nameLocalized.thirdLanguage
        )
          throw new BadRequestException('020,R20');
      }
    }

    // console.log('some test logging -> ', req.user.entityId, stockItemCode, writes);
    ...
    aggregation.push(
      {
        $match: { _id: this.toObjectId(newStockItem._id) },
      },
  ...
  async parseCsvFile(
    ...
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(file.buffer, originalname);
  ...
  async updateOneWarehoseStockItem(
    ...
    if (!stockItemSelected) throw new NotFoundException('stock item not found');
    ...
    // const superAdminsData = await this.usersService.m.find(
    //   { role: RolesEnum.SUPER_ADMIN },
    //   { email: 1, _id: 0 },
    // );

    // let superAdminsEmails: string[] = [];
    // superAdminsData.map(({ email }) => {
    //   // console.log('email from map', email);
    //   superAdminsEmails.push(email);
    // });

    const entity = await this.entitiesService.findOneById(entityId);
    await this.entitiesService.m.updateOne({ _id: entity._id }, { updatedAt: new Date() });

    // * send email to super admins
    // await this.mailService.sendUpdateStockItemEmail(superAdminsEmails, entity, stockItemSelected);
  ...
  async updateStockItem(
    ...
    const stockItemSelected = await this.aggregateOne([{ $match: { _id: stockitemId, entityId } }]);

    const updatedData = await this.updateOne({ _id: stockItemSelected._id }, { ...dto });
  ...
  async sendPurchaseOrderEmail(email: string, stockItem: string, qtyToOrder: number) {
  ...
  async sendExpiryStockEmail(email: string, name: string, stockItem: string, qtyToExpire: number) {
    const emailObject: EmailRequest = {
      to: email,
      from: '',
  ...
  async handleCron() {
    const stockItems = await this.m.aggregate([
      ...
      },
      {
        $match: {
          $expr: {
            $lte: ['$warehousestockitem.qtyOnHand', '$warehousestockitem.reOrderPoint'],
          },
        },
      },
      {
    ...
    for (const stockItem of stockItems) {
      ...
      const { warehouseId, qtyOnHand, qtyToOrder, reOrderPoint, purchasePrice } =
      ...
      if (preferredSupplierId) {
        const supplier = await this.suppliersService.findOneById(preferredSupplierId);
    ...
    await Promise.all(
      sortedDateBatches.map(async (stockItem) => {
        ...
        stockItem.batches.map(async (batches) => {
          const nearDate = Math.abs(Date.now() - new Date(batches.expiryDate).getTime());
  ...
  async calculateExpiryPrice(releaseQty: number, stockItem) {
    ...
    for (const stockItemDoc of stockItem) {
      ...
      if (releaseQty <= qtyRemaining && releaseQty != 0) {
        const stockitemQty = await this.stockItemTransaction.findById(stockItemDoc._id);
  ...
  async getPrice(
    ...
    if (pricing == 'FIFO') {
      ...
      // console.log('object -> ', sortedDateBatches);
  ...
  async updateIsFeatures(req: RequestWithUser, id: string, dto: UpdateIsFeaturesDto) {
  ...
  async itemWithZeroQtys(req: RequestWithUser, dto: ReportDto) {
    ...
    const allItemsAtSpecificPeriod = await this.find({
    ...
    for (const item in allItemsAtSpecificPeriod) {
      itemData = allItemsAtSpecificPeriod[item];
      ...
      for (const item in allWarehouseItemsAtSpecificPeriod) {
        ...
        if (itemQty == 0) items.push(itemData);
      }
    }
  ...
  async itemOutOfStock(req: RequestWithUser, dto: ReportDto) {
    let itemQty;
    let minQty;
    let itemData;
    let items = [];
    ...
    for (const item in allItemsAtSpecificPeriod) {
      ...
      for (const item in allWarehouseItemsAtSpecificPeriod) {
        itemQty = allWarehouseItemsAtSpecificPeriod[item].qtyOnHand;
        minQty = allWarehouseItemsAtSpecificPeriod[item].minQty;
        if (itemQty <= minQty) items.push(itemData);
      }
    }
