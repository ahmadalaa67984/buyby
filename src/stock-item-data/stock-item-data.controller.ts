...
export class StockItemDataController {
  ...
  async findAllInBranch(
    ...
    if (!req.user.type || req.user.type !== 'BUSINESS') {
      // // console.log("owner", req.user.owner);
  ...
  stockItemsCSV(@Req() req: RequestWithUser, @UploadedFile() file: any, @Res() res: Response) {
  ...
  updateMe(@Req() req: RequestWithUser, @Param('id') id: string, @Body() dto: UpdateStockItemDto) {
    return this.stockItemDataService.updateStockItem(
      this.stockItemDataService.toObjectId(id),
      req.user.entityId,
      dto,
      true,
  ...
  @Patch('update/:id/:entityId')
  @ApiOperation({ summary: 'Only super Admin could Update stock item by id' })
  @UseGuards(JwtAuthGuard, RoleGuard)
  ...
  @Roles(...RoleGroups.BUSSINESS)
  @ApiOperation({
    summary: 'search in middleware transaction table only for test',
  })
  ...
  @UseGuards(JwtAuthGuard, RoleGuard)
  @Roles(RolesEnum.CUSTOMER, RolesEnum.SUPER_ADMIN)
  findAllStockItems(
    @Req() req: RequestWithUser,
    @Body() options: SearchOptions,
  ): Promise<Pagination> {
    return this.stockItemCustomerDataService.findAllByCustomer(options);
  }
  ...
  findPopularStockItems(@Param('entityId') entityId: string, @Req() req: RequestWithUser) {
  ...
  async updateIsFeaturesDto(
    @Param('id') id: string,
    @Body() dto: UpdateIsFeaturesDto,
    @Req() req: RequestWithUser,
  ) {
  ...
  // toDo support full finding future
  ...
  async itemWithZeroQtys(@Req() req: RequestWithUser, @Body() dto: ReportDto) {
    return await this.stockItemDataService.itemWithZeroQtys(req, dto);
  }

  @Post('me/item-out-of-stock')
  @UseGuards(JwtAuthGuard, RoleGuard)
  @Roles(...RoleGroups.BUSSINESS)
  @ApiOperation({ summary: 'item out of stock' })
  async itemOutOfStock(@Req() req: RequestWithUser, @Body() dto: ReportDto) {
    return await this.stockItemDataService.itemOutOfStock(req, dto);
  }
}
