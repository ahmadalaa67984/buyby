...
import {
  ...
  CustomerOutput,
  GrossSalesOutput,
  LogsSystemDataOutput,
  NoMovingOutput,
  SlowMovingOutput,
  TopRecordsOutput,
} from './classes/reports-output.class';
...
export class ReportService extends BaseService<ReportDoc> {
  ...
  async prepareCustomerCSV(customersData: CustomerOutput[], sheetName: string, res: Response) {
    let modifiedCustomers = [];
    customersData.forEach((customer) => {
      console.log('ust', customer);
      let customersData = {
        ...
        'Date of Creation': customer.date,
      };
    ...
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, sheetName);
    res.status(200).json({ URL: `${uploadedFile.Location}` });
  }

  async prepareNoMovingCSV(noMovingSales: NoMovingOutput[], sheetName: string, res: Response) {
    let modifiedStockItems = [];
    noMovingSales.forEach((noMoving) => {
      const year = noMoving.createdAt.getFullYear();
      const month = (noMoving.createdAt.getMonth() + 1).toString().padStart(2, '0');
      const day = noMoving.createdAt.getDate().toString().padStart(2, '0');
      ...
      let modifyNoMoving = {
        ...
        itemCode: noMoving.itemCode,
        barCode: noMoving.barCode,
        'Creation Item Date': date,
      };
  ...
  async prepareSlowMovingCSV(noMovingSales: SlowMovingOutput[], sheetName: string, res: Response) {
    let modifiedStockItems = [];
    noMovingSales.forEach((noMoving) => {
      const year = noMoving.createdAt.getFullYear();
      const month = (noMoving.createdAt.getMonth() + 1).toString().padStart(2, '0');
  ...
  async prepareTopRecordsCSV(noMovingSales: TopRecordsOutput[], sheetName: string, res: Response) {
    let modifiedStockItems = [];
    noMovingSales.forEach((noMoving) => {
      const year = noMoving.createdAt.getFullYear();
      const month = (noMoving.createdAt.getMonth() + 1).toString().padStart(2, '0');
  ...
  async prepareGrossSalesCSV(noMovingSales: GrossSalesOutput[], sheetName: string, res: Response) {
    let modifiedStockItems = [];
    noMovingSales.forEach((noMoving) => {
      const year = noMoving.createdAt.getFullYear();
      const month = (noMoving.createdAt.getMonth() + 1).toString().padStart(2, '0');
      ...
      let modifyNoMoving = {
        ...
        itemCode: noMoving.itemCode[0],
        barCode: noMoving.barCode[0],
  ...
  async prepareDateComparison(sales, sheetName: string, res: Response) {
    let modifiedItems = [],
      modifyitem,
      date1,
      date2;
  ...
  async itemReport(options: ReportItemSalesOptions, entityId: Types.ObjectId, res: Response) {
    let aggregation = [],
      firstDateSales = null,
      comparison = null,
      dailyChartData = null,
      ...
      noMovingData = null,
      grossSalesData = null,
      avgBasketSizeData = null,
      itemDiscountData = null;
    ...
    if (comparisonByPeriod) {
      ...
      for (const filter of firstDateSales) {
        filter['date2'] = filter['date'];
        delete filter['date'];
        filter['totalAmount2'] = filter['totalAmount'];
    ...
    if (topRecords) {
      aggregation.push(
        ...
        },
        {
          ...
          $lookup: {
            from: 'stockcategories',
            localField: 'stockitem.stockCategoryId',
            foreignField: '_id',
            as: 'stockcategory',
          },
        },
        ...
        { $sort: { count: -1 } },
        {
          $project: {
            ...
            value: 1,
            itemCode: '$stockitem.stockItemCode',
      ...
      aggregation.push(
        ...
        { $match: { slowMoving: true } },
        {
          $project: {
            ...
            name: '$stockitem.nameLocalized.mainLanguage',
            slowMoving: 1,
            expectedMonthlyQtySold: '$stockitem.expectedMonthlyQtySold',
      ...
      aggregation.push(
        ...
        { $match: { noMove: true } },
        {
          $project: {
            ...
            noMovingDays: '$stockitem.slowMoving',
            _id: 0,
          },
        },
      );
  ...
  async addTransactionReport(options: DateFilter, entityId?: Types.ObjectId): Promise<any[]> {
    let count = 0;
    const aggregation = [];
  ...
  async auditTransactionReport(options: DateFilter, entityId?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
      ...
      aggregation.push(
        ...
        { $match: { slowMoving: true } },
        {
          $project: {
            ...
            name: '$stockitem.nameLocalized.mainLanguage',
            slowMoving: 1,
            'Expected Monthly Qty Sold': '$item.expectedMonthlySold',
  ...
  async reverseTransactionReport(options: DateFilter, entityId?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async shrinkageTransactionReport(options: DateFilter, entityId?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async wasteTransactionReport(options: DateFilter, entityId?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async deferredPayableReport(options: DateFilter, owner?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async deferredReceivableReport(options: DateFilter, owner?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async payable(options: DateFilter, owner?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async payableReport(dto: ReportDto, res: Response, owner?: Types.ObjectId) {
    const aggregation = [];
    ...
    // console.log('payable report', result);
    ...
    result.forEach((row) => {
      let modifyNoMoving = {
        ...
        DateOfCreation: row.createdAtToString,
      };
    ...
    const csvData = csvjson.toCSV(modifiedStockItems, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'payable.csv');

    res.status(200).json({ URL: `${uploadedFile.Location}` });
  }

  async receivableReport(dto: ReportDto, res: Response, owner?: Types.ObjectId) {
    const aggregation = [];
    ...
    // console.log('receivable report', result);

    let modifiedStockItems = [];
    result.forEach((row) => {
      let modifyNoMoving = {
        transactionType: row.transactionType,
        recievableAmount: row.recievableAmount,
        notes: row.notes,
        transactionID: row.transactionID,
        DateOfCreation: row.createdAtToString,
      };
    ...
    const csvData = csvjson.toCSV(modifiedStockItems, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'receivable.csv');

    res.status(200).json({ URL: `${uploadedFile.Location}` });
  }

  async receivable(options: DateFilter, owner?: Types.ObjectId): Promise<any[]> {
    const aggregation = [];
  ...
  async todayBudgetToTarget(req: RequestWithUser) {
    ...
    for (const order in todayOrders) {
      ...
      for (const itemOrder in itemOrders) {
        ...
        const items = await this.stockItemDataService.find({
          _id: itemOrders[itemOrder].stockItemId,
        });
        ...
        for (const item in items) {
          ...
          const itemsWarehoseData = await this.warehouseStockItems.find({
            _id: items[item].warehouseStockItemsData,
          });
    ...
    const entityExist = await await this.entitiesService.findOne({
      owner: this.toObjectId(req.user._id),
    });
    if (!entityExist) throw new NotFoundException('055,R055');
    const warehouseExist = await await this.warehouseService.findOne({
      _id: this.toObjectId(req.user.warehouseId),
    });
  ...
  async TopServiceSold(req: RequestWithUser, dto: ReportDto) {
    ...
    aggregation.push(
      ...
      },
      {
        $group: {
          ...
          createdAtToString: { $first: '$createdAtToString' },
        },
      },
      {
        $project: {
      ...
      { $sort: { count: -1 } },
      {
        $project: {
          ...
          totalPrice: 1,
          name: '$service.name',
          dateOfCreation: '$createdAtToString',
        },
      },
    );
    ...
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'topService.csv');
    return { URL: uploadedFile.Location };
  }

  async topItemsSold(req: RequestWithUser, dto: ReportDto, entityId?: Types.ObjectId) {
    let aggregation = [];

    if (entityId) aggregation.push({ $match: { entityId: { $eq: entityId } } });
    //item waste
    aggregation.push(
      {
        $addFields: {
...

      {
        $group: {
          _id: '$items.stockItemId',
          count: { $sum: 1 },
          totalPrice: { $sum: '$items.totalUnitPrice' },
          item: { $first: '$items.stockItemId' },
          dateOfCreation: { $first: '$createdAtToString' },
        },
      },
      {
        $unwind: { path: '$item', preserveNullAndEmptyArrays: false },
    ...
    let topItems = await this.orderService.m.aggregate(aggregation);
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'topItems.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async wastedItem(req: RequestWithUser, dto: ReportDto) {
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'waste.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async itemQtys(req: RequestWithUser, dto: ReportDto) {
    ...
    for (const item in allItemsAtSpecificPeriod) {
      ...
      barCode = allItemsAtSpecificPeriod[item].sku;
      let stockData = allItemsAtSpecificPeriod[item].warehouseStockItemsData;
      // console.log('stockData', stockData);

      const year = allItemsAtSpecificPeriod[item].createdAt.getFullYear();
      const month = allItemsAtSpecificPeriod[item].createdAt.getMonth() + 1;
      const day = allItemsAtSpecificPeriod[item].createdAt.getDate();
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'itemsQty.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async itemWithZeroQtys(req: RequestWithUser, dto: ReportDto) {
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'zeroQtyItems.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async itemMarginReport(req: RequestWithUser, dto: ReportDto) {
    ...
    for (const item in allItemsAtSpecificPeriod) {
      ...
      itemCode = allItemsAtSpecificPeriod[item].stockItemCode;
      barCode = allItemsAtSpecificPeriod[item].sku;
      // console.log('itemName', itemName, 'stockItemCode', itemCode, 'barcode', barCode);
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'margin.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async payablesAndDeferredPayablesLists(req: RequestWithUser, dto: ReportDto) {
    ...
    for (const payableItem in allPayablesAtSpecificPeriod) {
      ...
      // console.log('deferredPayables', allDeferredPayablesAtSpecificPeriod);
      ...
      for (const deferredPayableItem in allDeferredPayablesAtSpecificPeriod) {
        ...
        deferredPayable =
          allDeferredPayablesAtSpecificPeriod[deferredPayableItem].deferredPayableAmount;
        totalAmount = payable + deferredPayable;

        const year =
          allDeferredPayablesAtSpecificPeriod[deferredPayableItem].createdAt.getFullYear();
        const month =
          allDeferredPayablesAtSpecificPeriod[deferredPayableItem].createdAt.getMonth() + 1;
        const day = allDeferredPayablesAtSpecificPeriod[deferredPayableItem].createdAt.getDate();
  ...
  async recievablesAndDeferredrecievablesLists(req: RequestWithUser, dto: ReportDto) {
    ...
    for (const recievableItem in allRecievablesAtSpecificPeriod) {
      ...
      for (const deferredRecievableItem in allDeferredRecievablesAtSpecificPeriod) {
        ...
        deferredRecievable =
          allDeferredRecievablesAtSpecificPeriod[deferredRecievableItem].deferredRecievableAmount;
        totalAmount = recievable + deferredRecievable;
        const year = allRecievablesAtSpecificPeriod[recievableItem].createdAt.getFullYear();
        const month = allRecievablesAtSpecificPeriod[recievableItem].createdAt.getMonth() + 1;
        const day = allRecievablesAtSpecificPeriod[recievableItem].createdAt.getDate();
        const date = `${year}-${month}-${day}`;
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'recievables.csv');
    return { URL: uploadedFile.Location };
  }
  ...
  async logsList(options: ReportOptions, res: Response) {
    const aggregation = [];
  ...
  async businessAccountReport(req: RequestWithUser, dto: ReportDto) {
    ...
    const users = await this.usersService.find({
      ...
      role: { $in: RoleGroups.PROFILE },
      createdAt: {
        $gte: new Date(filterByDateFrom),
        $lte: new Date(filterByDateTo),
      },
    });

    const data = users.map((user) => {
      const { name, email, phoneNumber: phone, role: subscription, createdAt } = user;
      const year = createdAt.getFullYear();
      const month = createdAt.getMonth() + 1;
      const day = createdAt.getDate();
      const date = `${year}-${month}-${day}`;
      return { name, email, phone, subscription, date };
    });
    ...
    const modifiedData = data.map(({ name, email, phone, subscription, date }) => {
      return { name, email, phone, subscription, data };
    });
  ...
  async couponDetails(req: RequestWithUser, id: string) {
    const coupon = await this.couponService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
  ...
  async addTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.addTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
  ...
  async releaseTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.releaseTransactionsService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async wasteTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.wasteTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async aduitTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.auditTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async shrinkageTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.shrinkageTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async reverseTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.reverseTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async refundTransactionDetails(req: RequestWithUser, id: string) {
    const transactionDetails = await this.refundTransactionService.findOne(
      { owner: toObjectId(req.user._id), _id: toObjectId(id) },
      { owner: 0 },
    );
    if (!transactionDetails) throw new NotFoundException('019,R019');
  ...
  async warehousesList(req: RequestWithUser) {
    const warehouses = await this.warehouseService.find(
  ...
  async stockCategoryList(req: RequestWithUser) {
    const stockCategory = await this.stockCategoryService.find(
  ...
  async stockItemsList(req: RequestWithUser) {
    const stockItems = await this.stockItemDataService.find(
      { entityId: req.user.entityId },
      { entityId: 0 },
    );
    const entityExist = await await this.entitiesService.findOne({
  ...
  async servicesList(req: RequestWithUser) {
    const servicesList = await this.servicesService.find(
      { entityId: req.user.entityId },
      { entityId: 0 },
    );
  ...
  async subscription(req: RequestWithUser, dto: DateToDatePeriodDto) {
    ...
    for (const fCount in firstDateSubscription) {
      const user = await this.usersService.findOne({
        _id: { $in: [firstDateSubscription[fCount].ownerId[0]] },
    ...
    for (const sCount in secondDateSubscription) {
      const user = await this.usersService.findOne({
        _id: { $in: [secondDateSubscription[sCount].ownerId[0]] },
        status: StatusEnum.COMPLETED,
      });
      if (user) {
        packageName = user.subscriptionType;
    ...
    const result = {
      period: {
        'From Date': { date1: dto.firstDate, date2: dto.secondDate },
        'To Date': { date3: dto.thirdDate, date4: dto.fourthDate },
      },
      Data: Data,
    };
    ...
    const csvData = csvjson.toCSV(modifiedSubscriptions, { headers: 'key' });
    //upload to s3
    const uploadedFile: any = await this.uploadsService.s3Upload(csvData, 'subscription.csv');
    return { URL: uploadedFile.Location };
  }

  async statisticsReport(dto: ReportDto) {
    let storeObj: any = { entityType: EntityTypeEnum.STORE };
    let resObj: any = { entityType: EntityTypeEnum.RESTURANT };
    ...
    const stores = await this.entitiesService.count(storeObj);

    const restaurants = await this.entitiesService.count(resObj);

    const customers = await this.customerService.count(customersObj);

    const packages = await this.subscriptionService.find(packagesObj);
  ...
  async monthlyRevenue(dto: YearDto) {
    try {
      const { currentYear } = dto;
      ...
      return await this.subscriptionService.m.aggregate(aggregation);
    } catch (err: unknown) {
  ...
  async countMonthlyCreatedRes(dto: YearDto) {
    try {
      const { currentYear } = dto;
      ...
      return await this.entitiesService.m.aggregate(aggregation);
    } catch (err: unknown) {
      if (err instanceof Error) throw new BadGatewayException(err.message);
      else throw new BadGatewayException(String(err));
    }
  ...
  async countMonthlyCreatedStores(dto: YearDto) {
    try {
      const { currentYear } = dto;
  ...
  async countMonthlyCreatedCustomers(dto: YearDto) {
    try {
      const { currentYear } = dto;
      ...
      return await this.customerService.m.aggregate(aggregation);
    } catch (err: unknown) {
      if (err instanceof Error) throw new BadGatewayException(err.message);
      else throw new BadGatewayException(String(err));
    }
