...
export class CustomerService extends BaseService<CustomerDoc> {
  ...
  async createCustomer(req: RequestWithUser, dto: CreateCustomerDto) {
    ...
    try {
      await session.withTransaction(async (): Promise<any> => {
        ...
        if (!dto.email) dto.email = code + req.user.email;
      ...
      // console.log('errro from create customer', String(err));
  ...
  async updateCustomer(req: RequestWithUser, id: string, dto: UpdateCustomerDto) {
    if (req.user.status !== StatusEnum.COMPLETED) throw new BadRequestException('140,R140');
    const CustomerExist = await this.findOne({
      owner: { $in: [req.user._id] },
   ...
   */
  async findAll(owner: Types.ObjectId, options: CustomerSearchOptions): Promise<Pagination> {
    const aggregation = [];
